...
# Preprocessing: Add 'outcome' column for better pie chart labels
spacex_df['outcome'] = spacex_df['class'].apply(lambda x: 'Success' if x == 1 else 'Failed')
...
def get_scatter_chart(selected_site, payload_range):
    ...
    if selected_site == 'ALL':
        # If 'ALL' sites are selected, plot all filtered data
        ...
        filtered_site_payload_df = filtered_payload_df[
            filtered_payload_df['Launch Site'] == selected_site
        ]
...
if __name__ == '__main__':
    app.run(debug=True) # Change app.run_server to app.run